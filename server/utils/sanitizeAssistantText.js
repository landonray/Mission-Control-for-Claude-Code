...
// hallucination of its training). Persisting them is bad on two fronts:
//   1) they render to the user as if they were authoritative system messages
//   2) on session resume they get replayed to Claude and treated as real
// Strip them defensively from any assistant text we persist or display.
...
function findBoundary(remaining, fromPos) {
  ...
  return Math.min(...candidates) + 1; // skip the leading \n
}
  ...
  while (i < text.length) {
    ...
    if (withOutput) {
      ...
      if (boundary >= 0) {
        ...
        // No boundary — fall back to the line count.
        let pos = headEnd;
        for (let k = 0; k < n; k++) {
    ...
    // Looked like a tool marker but didn't match the structured format —
    // probably a literal "[Tool:" mention in prose. Keep it.
    result += '[Tool:';
    i = next + '[Tool:'.length;
  }
  return result;
...
module.exports = { sanitizeAssistantText, stripFakeHarnessTags };
